...
public class GTVGFilter implements Filter {
	...
	private boolean process(HttpServletRequest request,
		...
		try {
			...
			/*
			 * Query controller/URL mapping and obtain the controller that will
			 * process the request. If no controller is available, return false
			 * and let other filters/servlets process the request.
			 */
			IGTVGController controller = this.application
					.resolveControllerForRequest(request);
			if (controller == null) {
				return false;
			}
			 ...
			 */
			ITemplateEngine templateEngine = this.application
					.getTemplateEngine();
			...
			/*
			 * Execute the controller and process view template, writing the
			 * results to the response writer.
			 */
			controller.process(request, response, this.servletContext,
					templateEngine);
